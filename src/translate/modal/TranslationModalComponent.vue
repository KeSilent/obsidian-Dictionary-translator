<template>
	...
	>
		...
		<n-tabs type="line" animated placement="bottom">
				...
				<n-flex vertical>
					...
					<n-flex>
						<ul style="margin-left: -20px">
							<li
								v-for="explain in response.isWord
									? response.explains
			...
			<n-tab-pane
				name="chap2"
				:tab="plugin.i18n.t('web_site')"
				v-if="response.link && plugin.settings.show_link"
			>
				<n-flex>
					<iframe
						:src="response.link"
						class="translate-card-link"
					></iframe>
				</n-flex>
			</n-tab-pane>
			<n-tab-pane name="append" :tab="plugin.i18n.t('into_note')">
				<n-flex vertical>
					<n-flex vertical>
						<n-flex align="center">
							...
							>
								...
								{{
									isRecording
										? this.plugin.i18n.t("stop_record")
										: this.plugin.i18n.t("start_record")
								}}
							</n-button>
						</n-flex>
						<n-radio-group
							v-model:value="saveData.speech"
							name="speechRadio"
						>
							<n-flex>
								<n-radio
								...
								<n-radio
									v-for="(speech, index) in selfSpeech"
									@click="speechPlay(speech)"
									:key="index"
									:value="speech"
								>
									{{ `My Record ${index + 1}` }}
					...
					<n-flex vertical>
						...
						<n-flex
							v-for="(explain, index) in saveData.boomExplains"
							class="explain-card"
							vertical
						...
						>
							...
							<n-dynamic-tags
								v-model:value="explain.explains"
								:disabled="!explain.checked"
							/>
						</n-flex>
						<n-flex
						...
						>
							<n-flex>
								<n-checkbox
									v-model:checked="translation.checked"
								>
