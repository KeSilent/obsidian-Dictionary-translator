...
import {
	...
	to,
	TranslateEngines,
} from "./translate/const/translate-engines";
...
import {
	getLanguageOptions,
	support_lang,
} from "./translate/const/support-lang";
...
export class DictionarySettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const youdaoEngineDiv = containerEl.createDiv({
			cls: "setting-config-box",
		});
		youdaoEngineDiv.toggleClass(
			"settings-hide",
			this.plugin.settings.engine !== "youdao"
		);
		const youdaoHeader = youdaoEngineDiv.createDiv({
			cls: "setting-engine-header",
		});
		youdaoHeader.createEl("span", {
			text: i18n("guide", { engine: i18n("youdao") }) + ":",
			cls: "guide",
		});
		...
		new Setting(youdaoEngineDiv)
			...
			.addText((text) => {
				...
				text.setValue(
					`${
						(this.plugin.settings.engineConfig as YoudaoConfigs)
							.appKey ?? ""
					}`
				).onChange(async (value) => {
					this.setConfigValue(
						this.plugin.settings.engineConfig,
						"appKey",
						value
					);
		...
		new Setting(containerEl)
			...
			.addButton((bc) => {
				...
				bc.onClick(async (evt) => {
					...
					const translateResponse = await this.plugin
						.getTranslator()
						?.translate({
							from: en ? "cn" : "en",
							to: en ? "en" : "cn",
					...
					if (translateResponse) {
						new Notice(
							`${i18n("connect_test_success")} : ${words}  ➡️   ${
								translateResponse.translation
							}`
						);
					} else {
						new Notice(
							i18n("init_engine_exception", { error: "unknown" })
						);
					}
				});
			});
		...
		engine
			.setName(i18n("translate_engine"))
			.setDesc(i18n("engines_chooser_div_title"))
			.addDropdown((cb) =>
				cb
					.addOptions(this.getEnginesOptions())
		...
		const baseSettingDiv = containerEl.createDiv({
			cls: "setting-base-box",
		});
		const baseHeader = baseSettingDiv.createDiv({ cls: "setting-title" });
		baseHeader.createEl("span", {
			text: i18n("base_setting"),
			cls: "guide",
		});
		new Setting(baseSettingDiv)
			...
			.addDropdown((cb) =>
				cb
					.addOptions(getLanguageOptions(this.plugin.settings.lang))
		...
		new Setting(containerEl)
			...
			.addToggle((tc) => {
				tc.setValue(this.plugin.settings.show_link).onChange(
					async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((tc) => {
				tc.setValue(this.plugin.settings.show_radio).onChange(
					async (value) => {
		...
		new Setting(baseSettingDiv)
			.setName(i18n("word_patch"))
			.setDesc(i18n("word_patch_desc"))
			.addText((text) => {
				text.setPlaceholder(i18n("attach_placeholder"))
					.setValue(this.plugin.settings.word_patch)
					.onChange(async (value) => {
						this.plugin.settings.word_patch = value;
						await this.plugin.saveSettings();
					});
			});
	}
	...
	private setConfigValue<T, K extends keyof T, V extends T[K]>(
		obj: T | undefined,
		key: K,
		value: V
	): T {
